...
class NewsViewer:
    ...
    def view_all_news(self, limit=20, days_back=7):
        ...
        try:
            ...
            if not news_articles:
                print("\n⚠️ No news articles found in the specified time range.")
                return
    ...
    def view_news_by_stock(self, stock_symbol, limit=10, days_back=30):
        ...
        try:
            ...
            if not news_articles:
                print(f"\n⚠️ No news articles found for {stock_symbol.upper()} in the specified time range.")
                return
    ...
    def view_news_by_sentiment(self, sentiment_label='positive', limit=15, days_back=7):
        ...
        try:
            ...
            try:
                cursor.execute("""
                    ...
                    GROUP BY fn.news_id, fn.title, fn.content, fn.published_at, fn.url, 
                             ns.source_name, sa.sentiment_score, sa.confidence_score, sa.analysis_model
            ...
            if not news_articles:
                print(f"\n⚠️ No {sentiment_label} news articles found in the specified time range.")
                return
    ...
    def view_news_summary(self):
        ...
        try:
            ...
            # News sources
            cursor.execute("""
                SELECT ns.source_name, COUNT(fn.news_id) as article_count
                FROM news_sources ns
                LEFT JOIN financial_news fn ON ns.source_id = fn.source_id
    ...
    def export_news_to_dataframe(self, stock_symbol=None, days_back=7, sentiment_filter=None):
        ...
        try:
            from database_models import FinancialNews, NewsSource, SentimentAnalysis, StockNewsRelation, Stock
            ...
            query = session.query(
                ...
                NewsSource, FinancialNews.source_id == NewsSource.source_id
            ).outerjoin(
